    ...
    if not os.path.exists(DB_FILE):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS draws (
                ...
                ball1 INT, ball2 INT, ball3 INT, ball4 INT, ball5 INT, ball6 INT, bonus INT
            )
        """)
        ...
        if os.path.exists(CSV_FILE):
            ...
            if lines:
                count = 0
                for line in lines:
                    # استخراج كل الأرقام من السطر
                    nums = [int(n) for n in re.findall(r'\d+', line)]
                    
                    # استبعاد السنوات الأربعة (مثل 2023, 2024, 2025, 2026) وأرقام السحوبات الكبيرة (> 42)
                    lotto_numbers = [n for n in nums if 1 <= n <= 42]
                    
                    # أي سطر فيه 6 أرقام لوتو أو أكثر يعتبر سحبة صالحة
                    if len(lotto_numbers) >= 6:
                        balls = lotto_numbers[:6]
                        bonus = lotto_numbers[6] if len(lotto_numbers) >= 7 else 0
                        
                        cursor.execute(
                            "INSERT INTO draws (ball1, ball2, ball3, ball4, ball5, ball6, bonus) VALUES (?,?,?,?,?,?,?)",
                            balls + [bonus]
                        )
                        count += 1
