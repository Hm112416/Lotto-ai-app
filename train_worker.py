        ...
        if os.path.exists(CSV_FILE):
            ...
            if lines:
                ...
                for line in lines:
                    ...
                    # ترشيح الأرقام التي تقع في نطاق اللوتو (من 1 إلى 42)
                    valid_balls = [b for b in nums if 1 <= b <= 42]
                    
                    # إذا كان السطر يحتوي على 6 كرات على الأقل
                    if len(valid_balls) >= 6:
                        balls = valid_balls[:6]
                        bonus = valid_balls[6] if len(valid_balls) >= 7 else 0
                        
                        cursor.execute("INSERT INTO draws (ball1, ball2, ball3, ball4, ball5, ball6, bonus) VALUES (?,?,?,?,?,?,?)", balls + [bonus])
                        count += 1
