...
# 2. Time Machine
    ...
    seed_val = st.number_input("تغيير معامل العشوائية (Seed) للتنويع", min_value=1, max_value=9999, value=42)
    
    if st.button("🚀 تشغيل المحاكاة القوية"):
        with st.spinner("جاري حساب الفجوات وتدريب النموذج..."):
            ...
            rf = RandomForestClassifier(n_estimators=150, max_depth=12, random_state=int(seed_val))
            rf.fit(X, y)
...
# 4. Golden Coverage
    ...
    custom_seed = st.number_input("توليد تشكيلة جديدة (تغيير المعامل)", min_value=1, max_value=9999, value=np.random.randint(1, 1000))
    
    if st.button("🔮 حساب القائمة الذهبية وتوليد البطاقات"):
        X, y, matrix = build_advanced_features(df_all)
        rf = RandomForestClassifier(n_estimators=180, max_depth=14, random_state=int(custom_seed))
        rf.fit(X, y)
        ...
        st.subheader("🎯 البطاقات المفلترة والمجهزة للشراء:")
