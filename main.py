...
if cursor.fetchone()[0] == 0:
    ...
    for i in range(1, 101):
        ...
        sample_draws.append((int(i), f"2024-01-{(i%28)+1:02d}", int(balls[0]), int(balls[1]), int(balls[2]), int(balls[3]), int(balls[4]), int(balls[5]), int(bonus)))
    cursor.executemany('''
...
# 3. Time Machine
    ...
    target_draw = st.number_input("رقم السحبة المراد اختبارها", min_value=10, max_value=max_d, value=min(50, max_d))
