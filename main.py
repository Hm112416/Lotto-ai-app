...
# Consensus Engine across multiple models
def get_consensus_probabilities(X, y, matrix, num_draws_total):
    combined_probs = np.zeros(42)
    seeds = [10, 42, 100, 2024, 999]
    
    last_i = num_draws_total
    ...
    X_target = np.concatenate([freq_s, freq_m, freq_l, delays, matrix[-1], matrix[-2]]).reshape(1, -1)
    
    for s in seeds:
        rf = RandomForestClassifier(n_estimators=120, max_depth=12, random_state=s)
        rf.fit(X, y)
        probas = rf.predict_proba(X_target)
        
        for idx in range(42):
            if probas[idx].shape[1] > 1:
                combined_probs[idx] += probas[idx][0, 1]
                
    return combined_probs / len(seeds)

st.sidebar.title("🔥 Lotto AI Consensus Engine")
st.sidebar.markdown(f"**عدد السحوبات:** `{len(df_all)}` سحبة")
st.sidebar.markdown("---")

menu = st.sidebar.radio("القائمة الرئيسية", [
    "📊 تحليل الكثافة والفجوات",
    "🕰️ آلة الزمن واختبار الإجماع",
    "⚙️ مشغل التدريب المستمر 24/24",
    "🎫 توليد القائمة الذهبية الموحدة"
])
...
# 2. Time Machine
elif menu == "🕰️ آلة الزمن واختبار الإجماع":
    st.header("🕰️ آلة الزمن بنظام الإجماع الموحد")
    ...
    if st.button("🚀 تشغيل المحاكاة القوية"):
        with st.spinner("جاري تدريب 5 نماذج مجتمعة وحساب الإجماع..."):
            ...
            probs = get_consensus_probabilities(X, y, matrix, len(past_df))
            ...
            st.write(f"🔥 **التغطية بـ 18 رقم (الذهبية الموحدة):** `{top_18}` ⬅️ أصابت **{h18}** من 6! 🎉")
...
# 4. Golden Consensus Coverage
elif menu == "🎫 توليد القائمة الذهبية الموحدة":
    st.header("🎫 القائمة الذهبية والبطاقات المجهزة")
    st.info("يتم الآن حساب القائمة بناءً على إجماع 5 نماذج ذكاء اصطناعي قوية لمنع التشتت.")
    
    if st.button("👑 استخراج أفضل قائمة وبطاقات موحدة"):
        with st.spinner("جاري دمج نتائج الخوارزميات وتوليد التشكيلة الأقوى..."):
            X, y, matrix = build_advanced_features(df_all)
            probs = get_consensus_probabilities(X, y, matrix, len(df_all))
            
            top_18 = clean_list(sorted(np.argsort(probs)[-18:] + 1))
            st.success(f"👑 **الـ 18 رقم الذهبية النهائية (إجماع النماذج):**\n\n`{top_18}`")
            st.markdown("---")
            
            tickets = []
            attempts = 0
            # Fixed seed for tickets generation to ensure strict stability
            np.random.seed(42)
            ...
            st.subheader("🎯 البطاقات الـ 6 الموصى بها رسمياً:")
